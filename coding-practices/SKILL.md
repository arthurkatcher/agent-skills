...
- Unsure about an API surface, a signature, or a version difference → read the installed source first; if that does not settle it, use the docs-lookup skill in this workspace at `.agent/skills/docs-lookup/SKILL.md`: run `bash .agent/skills/docs-lookup/scripts/docs.sh search <library>` to get the library id, then `bash .agent/skills/docs-lookup/scripts/docs.sh get <id> "<topic>"` for the snippets. One call per problem; quote the line you relied on. Never write against memory and hope.
